...
const Banesco_displayOverlay = (Banesco_displayBool) => {
...
const Banesco_Cancelar = (event) => {
    if (event.origin !== "https://localhost:5001")
        return;

    if (event.data === "banesco_pago_completo" || event.data === "banesco_cancelar") {
        var banesco_iframe = document.getElementById("Banesco_DIVIframe");
        document.body.removeChild(banesco_iframe);
        window.removeEventListener("message", Banesco_Cancelar, false);
    }
}

const Banesco_boton = (url, cedula, monto, idtramite, comprobante, concepto, apikey, firma, tipo) => {
    return `<img id='banesco_img_boton' src='data:image/gif;base64, ` `'  onclick='Banesco_open_win("` + url + `","`
        + cedula + `","` + monto + `","`
...
const Banesco_open_win = (url, cedula, monto, idtramite, comprobante, concepto, apikey, firma, tipo) => {

    // if (tipo === 1) {
        var Banesco_window = window.open("Boton de Pago Banesco", "Banesco_Boton", "dialog=yes,status=no,toolbar=no,location=no,menubar=no,scrollbars=no,resizable=no,fullscreen=no,height=" + (window.screen.height * 0.8) + "px, width = 465px");
        //var loading= _LOADING_;
        var html = "<div style='background-color:#00795354;height:100%;'><div style='top:50%;left:35%;position:absolute;'><img src='data:image/gif;base64,' /><br><span>Conectando ...</span></div></div>";
